...
sub BUILD {
    my $self = shift;
    $self->routine(
        __create_piface_input_routine( $self->piface, $self->channel ) );
...
sub __create_piface_input_routine {
    ...
    my $input_code_ref = sub {
        ...
        while (1) {
            ...
            if ( $input != $last_input ) {
                $channel->send(
                    { 'input' => $input, 'last_input' => $last_input } );
                $last_input = $input;
            }
            sleep(0.01);
        }
    ...
    return Notifier::Routine::create_input_routine( $channel, $input_code_ref,
        $on_finish_ref );
}
